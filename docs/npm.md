...
This monorepo contains two main packages: `@zmsfa/cli` and
`@zmsfa/core`.
...
with `npm install -g @zmsfa/cli` or runs it directly with
`npx @zmsfa/cli`, they are using this single, self-contained executable.
...
- **Simplified script execution**: You can run scripts in any package from the
  ...
  `npm run build --workspace @zmsfa/cli`.
